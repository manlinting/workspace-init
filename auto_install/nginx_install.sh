...
CONFIG="--with-pcre=../pcre-8.21 --with-http_ssl_module --with-openssl=../openssl-1.0.1a"
...
set -x 
cp $WORKDIR/conf/nginx.conf $INSTALL_DIR/conf/nginx.conf
localip=$(get_localip)
sed -i "s/127.0.0.1:80/$localip:80/g" $INSTALL_DIR/conf/nginx.conf


logmsg "Starting nginx..."
$INSTALL_DIR/sbin/nginx  >> "$LLLOG"  2>&1
if ps aux|grep httpd |grep -v grep ;then 
  logmsg "nginx started"
else 
  logmsg "nginx start failed!"
fi
