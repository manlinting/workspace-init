...
CONFIG="--enable-so --enable-rewrite"
...
## check if apache is runing
if netstat --inet -lpn 2>/dev/null | grep --color httpd; then
    die "apache maybe running ..."
fi
if netstat --inet -lpn 2>/dev/null | grep --color ':80'; then
    die "PORT 80 is listening ..."
fi
...
cp $WORKDIR/conf/httpd.conf $INSTALL_DIR/conf/
localip=$(get_localip)
sed -i "s/127.0.0.1:80/$localip:80/g" $INSTALL_DIR/conf/httpd.conf


logmsg "Starting apache..."
$INSTALL_DIR/bin/apachectl -k start >> "$LLLOG"  2>&1
if ps aux|grep httpd |grep -v grep ;then 
  logmsg "apache started"
else 
  logmsg "apache start failed!"
fi
